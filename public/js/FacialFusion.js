...
function render() {
	$.ajax({
		...
		success: (res) => {
			...
			$('.template').html(html);
			$('.main')
				.css('background', `url(${res[0]}) center top /cover`)
				.data('imgData', res[0]);
		},
		error: (xhr) => {
			$('.message')
				.css({ 'background-color': 'rgb(247, 211, 211)', color: ' #721c24' })
	...
	$('.template').on('mouseup touchend', 'div', function (e) {
		...
		if (!flag) {
			...
			$('.main')
				.css(
					'background',
					`${$(this).css('backgroundImage')} center top /cover`
				)
				.data('imgData', $(this).attr('data-imgData'));
		}
		flag = false;
	});
		...
		fileRead.onload = function () {
			...
			try {
				...
				localStorage.setItem(
					'img_num',
					parseInt(localStorage.getItem('img_num')) + 1
				);
			} catch (err) {
				$('.message')
					.css({ 'background-color': 'rgb(247, 211, 211)', color: ' #721c24' })
					.html('<strong>存储空间一不小心用光了，删除一些图片吧</strong>')
					.addClass('action');
				setTimeout(() => {
					$('.message').removeClass('action');
				}, 3000);
	...
	$('.selectInput').on('change', function (e) {
		...
		fileRead.onload = function () {
			...
			$.ajax({
				...
				url: '/getface',
				data: {
					tplpath: $('.main').data('imgData'),
					wrappath: fileRead.result,
				},
				...
				success: (res) => {
					...
					$('.main').css(
						'background',
						`url(${'data:image/jpeg;base64,' + res}) center top /cover`
					);
				...
				error: (xhr) => {
					...
					$('.message')
						...
						.html(
							'<strong>' + JSON.parse(xhr.responseText).message + '</strong>'
						)
						.addClass('action');
					setTimeout(() => {
						$('.message').removeClass('action');
